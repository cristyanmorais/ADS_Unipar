...
public class Vendas extends javax.swing.JPanel {
                    ...
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                        .addComponent(valorTotalTextField, javax.swing.GroupLayout.DEFAULT_SIZE, 200, Short.MAX_VALUE)
    ...
    private void adicionarItemVenda() {
        ...
        long indiceProdutoSelecionado = produtosComboBox.getSelectedIndex();
        ...
        try {
            ...
            float valorTotalFloat = valorUnitarioFloat * quantidadeInt;
            ...
            String selectedItem = (String) produtosComboBox.getSelectedItem();
            Produto produto = new Produto(indiceProdutoSelecionado, selectedItem);
            
            ItemVenda itemVenda = new ItemVenda();
            itemVenda.setQuantidade(quantidadeInt);
            itemVenda.setValor_unitario(valorUnitarioFloat);
            itemVenda.setValor_total(valorTotalFloat);
            itemVenda.setProduto(produto);
            
            itensVenda.add(itemVenda);
            
            valorTotalVenda += valorTotalFloat;
            
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Os campos 'Quantidade' e 'Valor Unitário' devem ser números.", "Erro", JOptionPane.ERROR_MESSAGE);
        }
        
        valorTotalTextField.setText(String.valueOf(valorTotalVenda));
        
        itemVendaTableModel.setItensVenda(itensVenda);
        
        // Limpar Campos
        produtosComboBox.setSelectedIndex(0);
        quantidadeTextField.setText("0");
        valorUnitarioTextField.setText("0");
    }
...
//        }
        ...
        Cliente cliente = new Cliente(Long.valueOf(clientesComboBox.getSelectedIndex()));
        
        Venda venda = new Venda(observacoesTextField.getText(), dataTextField.getText(), valorTotalVenda, cliente);
        
        try {
            long vendaId = insertVenda(venda);
            venda.setId(vendaId);
            
            for (ItemVenda item : itensVenda) {
                item.setVenda(venda);
                insertItemVenda(item);
            }
            ...
            System.out.println("Erro: " + extractErrorMessage(e));
    ...
    private Produto getProdutoById(long id) throws IOException {
        URL url = new URL("http://localhost:8080/produto/{id}");
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("GET");
        
        InputStreamReader reader = new InputStreamReader(conn.getInputStream());
        Gson gson = new Gson();
        return gson.fromJson(reader, Produto.class);
    }
    
    private void insertItemVenda(ItemVenda itemVenda) throws IOException {
        URL url = new URL("http://localhost:8080/item-venda/insert");
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        ...
        Gson gson = new Gson();
        String jsonInputString = gson.toJson(itemVenda);
        ...
        if (responseCode == HttpURLConnection.HTTP_OK || responseCode == HttpURLConnection.HTTP_CREATED) {
            ...
            try (InputStream es = conn.getErrorStream()) {
                if (es != null) {
                    try (InputStreamReader reader = new InputStreamReader(es, "utf-8")) {
                    ...
                    throw new IOException("Erro desconhecido ao processar resposta do servidor: " + responseMessage);
                }
            }
        }
    }
    
    private long insertVenda(Venda venda) throws IOException {
        URL url = new URL("http://localhost:8080/venda/insert");
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("POST");
        conn.setRequestProperty("Content-Type", "application/json");
        conn.setDoOutput(true);
        ...
        Gson gson = new Gson();
        String jsonInputString = gson.toJson(venda);
        ...
        if (responseCode == HttpURLConnection.HTTP_OK || responseCode == HttpURLConnection.HTTP_CREATED) {
            try (InputStream is = conn.getInputStream()) {
                if (is != null) {
                    ...
                    throw new IOException("Erro ao obter resposta do servidor.");
                }
            }
        } else {
            try (InputStream es = conn.getErrorStream()) {
